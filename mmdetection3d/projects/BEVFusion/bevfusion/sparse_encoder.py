...
class BEVFusionSparseEncoder(SparseEncoder):
    ...
    def forward(self, voxel_features, coors, batch_size):
        ...
        # voxel_indices = coors[:, 1:]  # [z, y, x]
        # valid_z = (voxel_indices[:, 0] >= 0) & (voxel_indices[:, 0] < self.sparse_shape[0])
        # valid_y = (voxel_indices[:, 1] >= 0) & (voxel_indices[:, 1] < self.sparse_shape[1])
        # valid_x = (voxel_indices[:, 2] >= 0) & (voxel_indices[:, 2] < self.sparse_shape[2])

        # valid_mask = valid_z & valid_y & valid_x
        # print(len(coors), len(valid_mask))
