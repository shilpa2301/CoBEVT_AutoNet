...
class TransFusionHead(nn.Module):
    ...
    def forward_single(self, inputs, metas):
        ...
        if self.test_cfg['dataset'] == 'opv2v':
        ...
        if self.test_cfg['dataset'] == 'opv2v_':
            self.query_labels = torch.zeros_like(top_proposals)
            one_hot = F.one_hot(
                torch.zeros_like(top_proposals),
                num_classes=self.num_classes).permute(0, 2, 1)
        else:
            top_proposals_class = top_proposals // heatmap.shape[-1]
            self.query_labels = top_proposals_class
        ...
        # heatmaps = dense_heatmap.squeeze(0)

        # # Convert to numpy for plotting
        # heatmaps_np = heatmaps.cpu().numpy()

        # # Create a subplot for each heatmap
        # num_classes = heatmaps_np.shape[0]
        ...
        #     # Show the heatmap using a colormap (e.g., 'viridis')
        #     im = ax.imshow(heatmaps_np[i], cmap='viridis')
        ...
        # plt.tight_layout()
        # plt.savefig("/home/csmaj/jeli/MultiModalityPerception/opv2v/opencood/images/heatmaps/heatmaps_visualization.png")
    ...
    def predict_and_loss(self, feats, metas, batch_dict):
        """Forward pass.
        ...
        # Calculate loss
        gt_bbox = batch_dict['gt_box3d'][batch_dict['gt_box3d_mask'] == 1]
        # if gt_bbox.shape[1] == 7:
        #     gt_bbox = torch.cat((gt_bbox.to(device), torch.zeros(gt_bbox.shape[0], 2).to(device)), dim=1)
        gt_bbox = LiDARInstance3DBoxes(gt_bbox)
        gt_labels = torch.ones(int(batch_dict['gt_box3d_mask'].sum()))
        metainfo = {}
        instance_data = InstanceData()
        instance_data.bboxes_3d = gt_bbox.to(device)
        instance_data.labels_3d = gt_labels.long().to(device)
        batch_gt_instances_3d = []
        batch_gt_instances_3d.insert(0, instance_data)

        pred_dict = dict()
        score = copy.deepcopy(res[0][0]['heatmap'].detach())
        center = copy.deepcopy(res[0][0]['center'].detach())
        height = copy.deepcopy(res[0][0]['height'].detach())
        dim = copy.deepcopy(res[0][0]['dim'].detach())
        rot = copy.deepcopy(res[0][0]['rot'].detach())
        if 'vel' in res[0][0].keys():
            vel = copy.deepcopy(res[0][0]['vel'].detach())
        else:
            vel = None
        ...
        pred_dict['bboxes_3d'] = temp[0]['bboxes']
        pred_dict['scores_3d'] = temp[0]['scores']
        pred_dict['labels'] = temp[0]['labels']

        # print(pred_dict['bboxes_3d'], pred_dict['bboxes_3d'].shape)
        
        loss, pred = self.loss_by_feat(res, batch_gt_instances_3d)
        pred_dict['bboxes_3d'] = pred[0]
    ...
    def predict(self, batch_feats, batch_input_metas):
        ...
        res = self.predict_by_feat_v2(preds_dicts)
        return res
    ...
    def predict_by_feat(self,
        ...
        for layer_id, preds_dict in enumerate(preds_dicts):
            ...
            elif self.test_cfg['dataset'] == 'opv2v':
                self.tasks = [
                    dict(
                        num_class=1,
                        class_names=['Car'],
            ...
            for i in range(batch_size):
                ...
                temp_instances.bboxes_3d = metas['box_type_3d'](
    ...
    def predict_by_feat_v2(self, preds_dicts: Tuple[List[dict]]):
        """Forward pass for prediction only, mimicking loss_by_feat without loss calculation.
        
        Args:
            preds_dicts (Tuple[List[dict]]): Prediction dictionaries
            
        Returns:
            InstanceData: Predictions containing bboxes_3d, scores_3d and labels_3d
        """
        preds_dict = preds_dicts[0][0]
        
        # Get predictions for the last layer (or only layer if auxiliary is False)
        idx_layer = self.num_decoder_layers - 1 if self.auxiliary else 0
        
        # Calculate scores 
        batch_score = preds_dict['heatmap'][..., idx_layer * 
                                        self.num_proposals:(idx_layer + 1) * 
                                        self.num_proposals].sigmoid()
        
        # Apply query heatmap score and one_hot mask
        one_hot = F.one_hot(
            self.query_labels,
            num_classes=self.num_classes).permute(0, 2, 1)
        ...
        # Truncate to 8 dimensions if needed
        if self.bbox_coder.code_size == 8:
            preds = preds[:, :, :8]
        
        # Decode predictions using bbox_coder
        decoded_bboxes = self.pred_bbox_decode(preds.squeeze(0))
        
        # Get scores and labels
        scores = batch_score.max(1)[0].squeeze(0)  # Get max score across classes
        labels = batch_score.max(1)[1].squeeze(0)  # Get class with max score
        
        # Create InstanceData object
        results = InstanceData()
        results.bboxes_3d = decoded_bboxes
        results.scores_3d = scores 
        results.labels_3d = labels
        
        return [results]
    
    def pred_bbox_decode(self, bbox_targets):
        # decode the pred bbox from the box coder 
        targets = torch.zeros([bbox_targets.shape[0], 7]).to(bbox_targets.device)
        targets[:, 0] = (bbox_targets[:, 0] * (self.bbox_coder.out_size_factor * self.bbox_coder.voxel_size[0])) + self.bbox_coder.pc_range[0]
        targets[:, 1] = (bbox_targets[:, 1] * (self.bbox_coder.out_size_factor * self.bbox_coder.voxel_size[1])) + self.bbox_coder.pc_range[1]
        # targets[:, 2] = (dst_boxes[:, 2] - self.post_center_range[2]) / (self.post_center_range[5] - self.post_center_range[2])
        targets[:, 3] = bbox_targets[:, 3].exp()
        targets[:, 4] = bbox_targets[:, 4].exp()
        targets[:, 5] = bbox_targets[:, 5].exp()
        targets[:, 2] = bbox_targets[:, 2] - targets[:, 5] * 0.5  # bottom center to gravity center
        targets[:, 6] = torch.atan2(bbox_targets[:, 6], bbox_targets[:, 7])
    ...
    def get_targets_single(self, gt_instances_3d, preds_dict, batch_idx):
        ...
        for idx_layer in range(num_layer):
            ...
            if gt_bboxes_tensor.shape[1] == 7:
                bboxes_tensor_layer = bboxes_tensor_layer[:,:7]
            ...
            # examine assign_result.max_overlaps is None
            if assign_result.max_overlaps is None:
                assign_result.max_overlaps = torch.zeros(bboxes_tensor_layer.shape[0], device=score.device)
        ...
        # and iou loss
        if len(pos_inds) > 0:
            # print("Before encoder: ", gt_bboxes_tensor, gt_bboxes_tensor.shape)
            pos_bbox_targets = self.bbox_coder.encode(
                sampling_result.pos_gt_bboxes)
            # print("After encoder: ", pos_bbox_targets, pos_bbox_targets.shape)
        ...
        # print("groundtruth predict and loss:", bbox_targets)
    ...
    def loss_by_feat(self, preds_dicts: Tuple[List[dict]],
        ...
        for idx_layer in range(
            ...
            # loss_dict[f'{prefix}_loss_iou'] = torch.tensor(0.0) # layer_loss_iou
            loss_dict[f'loss_cls'] = layer_loss_cls
            loss_dict[f'loss_bbox'] = layer_loss_bbox
            loss_dict[f'loss_iou'] = torch.tensor(0.0) # layer_loss_iou
