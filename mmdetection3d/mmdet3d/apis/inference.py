...
import sys
sys.path.append('/workspace/mmdetection3d/')
sys.path.append('/home/jerryli/MultiModalityPerception/mmdetection3d/mmdet3d')
...
def inference_multi_modality_detector(model: nn.Module,
    ...
    for index, pcd in enumerate(pcds):
        ...
        img = imgs[index]
        print("images", data_list[0]["images"])
        if cam_type != 'all':
        ...
        # results = data_info
        # for step in test_pipeline.transforms:
        #     print(f"Before step {step}: {results.keys()}")
        #     results = step(results)
        #     print(f"After step {step}: {results.keys()}")

        data_ = test_pipeline(data_info)
        data.append(data_)
