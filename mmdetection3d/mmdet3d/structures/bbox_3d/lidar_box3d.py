...
class LiDARInstance3DBoxesVelocity(BaseInstance3DBoxes):
    """LiDARInstance3DBoxes for LiDAR data, including velocity.

    Args:
        tensor (Tensor or np.ndarray or Sequence[Sequence[float]]): The boxes
            data with shape (N, box_dim), where box_dim should be 9.
        box_dim (int): Number of the dimension of a box. Each row is
            (x, y, z, x_size, y_size, z_size, yaw, vx, vy). Defaults to 9.
    ...
    def __init__(
        ...
        assert box_dim == 9, 'LiDARInstance3DBoxesVelocity requires box_dim to be 9'
        super().__init__(tensor, box_dim=box_dim, with_yaw=with_yaw, origin=origin)

    @property
    def velocity(self) -> Tensor:
        """Tensor: A vector with velocity of each box in shape (N, 2)."""
        return self.tensor[:, 7:9]

    def convert_to(self, dst_format: str) -> 'LiDARInstance3DBoxesVelocity':
        """Convert the box format to the target format.

        This method is a placeholder if you need to convert the box to another format.
        For example, converting LiDARInstance3DBoxes to CameraInstance3DBoxes.

        Args:
            dst_format (str): The target format.

        Returns:
            LiDARInstance3DBoxesVelocity: The converted boxes.
        """
        # Here you can define how to convert to other formats if needed
        raise NotImplementedError('Conversion method is not implemented.')

    def clone(self) -> 'LiDARInstance3DBoxesVelocity':
        """Clone the boxes.

        Returns:
            LiDARInstance3DBoxesVelocity: The cloned boxes.
        """
        return LiDARInstance3DBoxesVelocity(self.tensor.clone(), box_dim=self.box_dim, with_yaw=self.with_yaw)