...
class LoadPointsFromFile(BaseTransform):
    ...
    def _load_points(self, pts_filename: str) -> np.ndarray:
        ...
        try:
            pts_bytes = get(pts_filename, backend_args=self.backend_args)
            # pts_bytes = pts_bytes[0:(len(pts_bytes) // np.dtype(np.float32).itemsize)*np.dtype(np.float32).itemsize]
            points = np.frombuffer(pts_bytes, dtype=np.float32)
        except ConnectionError:
            mmengine.check_file_exist(pts_filename)
            if pts_filename.endswith('.npy'):
    ...
    def transform(self, results: dict) -> dict:
        ...
        pts_file_path = "/home/bowu/Project/SensorPlatform/pipeline/submodules/mmdet3d/demo/data/nuscenes/n015-2018-07-24-11-22-45+0800__LIDAR_TOP__1532402927647951.pcd.bin"
