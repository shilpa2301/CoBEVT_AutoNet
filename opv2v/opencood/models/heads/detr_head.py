...
from mmdet.models.losses import FocalLoss, L1Loss, GIoULoss, IoULoss
MODELS.register_module(name='FocalLoss', module=FocalLoss)
MODELS.register_module(name='L1Loss', module=L1Loss)
MODELS.register_module(name='GIoULoss', module=GIoULoss)
MODELS.register_module(name='IoULoss', module=IoULoss)


from opencood.models.heads.detr_head_plugin.anchor_free_head import AnchorFreeHead
...
class DETRHead_old(AnchorFreeHead):
    ...
    def __init__(self,
                 ...
                 train_cfg=dict(
                     assigner=dict(
                        ...
                        #      dict(type='BBoxL1Cost', weight=0.25),
        ...
        if train_cfg:
            ...
            # assigner['match_costs']['ClassificationCost'] = loss_cls['loss_weight']
            # assigner['match_costs']['BBoxL1Cost'] = loss_bbox['loss_weight']
            # assigner['match_costs']['IoUCost'] = loss_iou['loss_weight']
            # print("loss_cls['loss_weight']:", loss_cls['loss_weight'])
            # print("assigner['cls_cost']['weight']:", assigner['cls_cost']['weight'])
            ...
            self.sampler = TASK_UTILS.build(sampler_cfg, default_args=dict(context=self))
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is DETRHead_old:
            convert_dict = {
